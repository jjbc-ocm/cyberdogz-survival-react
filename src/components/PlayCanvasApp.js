...
// }, [wallet.wallet]);
  ...
  // const payTokenOld = async (amount) => {
  ...
  //     //const toWalletAddress = new PublicKey('5cog2CULu9HQhH2vC51Qwpdhp2LoQiAGd8VwGi36C8s7');
  ...
  const payToken = async (amount) => {
    try {
      ...
      const tokenMintAddress = new PublicKey('G4USAGAtyEfW6B5abRRNHnvn1x2Vhcxhfvdv2h3m24iW'); // CBZ
      const connection = new Connection(endpoint, 'confirmed');
      const fromTokenAccount = await getAssociatedTokenAddress(tokenMintAddress, fromWalletAddress);
      const toTokenAccount = await getAssociatedTokenAddress(tokenMintAddress, toWalletAddress);
      const latestBlockhash = await connection.getLatestBlockhash();
